...
\begin{document}
  ...
  \begin{define}[Opticsの圏]\label{def-cat-of-optics}
    ...
    \begin{quote}
			\begin{mydescription}
				\item[対象] 対象の集合を$\obj{Optic_C}=\obj{C}\times \obj{C}$とする。
				\item[射] 任意の対象$(S,S'), (A,A')$に対する射集合を\[\arset{Optic_C}{(S,S')}{(A,A')}=\coend{C}{M}\arset{C}{S}{M\otimes A}\times\arset{C}{M\otimes A'}{S'}\]とする。
				\item[射の合成] 定義\ref{def-comp-of-optic}を射の合成とする。
				\item[恒等射の存在] 定義\ref{def-identity-optic}を恒等射とする。
				\item[結合律] 命題\ref{prop-associativity-of-optic}より成り立つ。
				\item[単位元律] 命題\ref{prop-optic-has-identity}より成り立つ。
			\end{mydescription}
		\end{quote}
  \end{define}
